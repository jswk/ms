...
public class HmsCmaEsOC extends HgsOrchestratorConfigurer {
    ...
    protected void doConfigure(HgsOrchestrator orch) {
        ...
        state.setPopulationSizes(new int[] { 40, 4 + (int) floor(3 * log(dim)) });
        ...
        final EvoAlg[] eas = new EvoAlg[] {
                new SEA(0.1, 0.5, 8.0 / dimMultiplier, orch.rand, orch.fitnessExtractor),
                new CMAES(0.5, -1, orch.rand, orch.individualEvaluator, orch.fitnessExtractor)
        };
        final LocalStopCondition[] lscs = new LocalStopCondition[] {
                new TrivialLSC(),
                new NoObjectiveChangeLSC(3, ArraysUtil.constant(dim, 1e-2))
        };
        final Sprouter[] sprouters = new Sprouter[] {
                new BasicSprouter(0.5, 0., 1, orch.rand),
                new NoSprouter()
        };
        final SproutReducer[] sproutReducers = new SproutReducer[] {
                new ChainedSproutReducer(
                        new DemeDistanceSproutReducer(1),
                        new SeedDistanceSproutReducer(1)),
                new NoSproutReducer()
        };
