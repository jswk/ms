...
import pl.a2s.ms.core.lsc.OrLSC;
...
public class HmsSea2LevelOC extends HgsOrchestratorConfigurer {
    ...
    protected void doConfigure(HgsOrchestrator orch) {
        ...
        state.setPopulationSizes(new int[] { 40, 20 });
        state.setGlobalStopCondition(OrGSC.of(new NoDemesRunningGSC()));

        final Level[] hgsDemes = new Level[2];
        final double dimMultiplier = 1; // Math.pow(dim, .5);
        final double[] stds = new double[] { 4.0, 0.4 };
        final EvoAlg[] eas = new EvoAlg[] {
                new SEA(0.1, 0.5, stds[0] / dimMultiplier, orch.rand, orch.fitnessExtractor),
                new SEA(0.5, 0.5, stds[1] / dimMultiplier, orch.rand, orch.fitnessExtractor)
        };
        final LocalStopCondition[] lscs = new LocalStopCondition[] {
                new NoSproutLSC(5),
                OrLSC.of(new NoObjectiveChangeLSC(3, ArraysUtil.constant(dim, 1e-3)),
                         new EpochCountLSC(100))

        };
        final Sprouter[] sprouters = new Sprouter[] {
                new BasicSprouter(0.5, stds[1] / dimMultiplier, state.getPopulationSizes()[1], orch.rand),
                new NoSprouter()
        };
