...
public abstract class Report1902AbstractOC extends HgsOrchestratorConfigurer {

    @Setter private int budget = 100;
    ...
    protected void doConfigure(HgsOrchestrator orch) {
        ...
        state.setPopulationSizes(new int[] { 20, 16, 10 });
        ...
        final double[] stds = new double[] { 1.0, 0.5, 0.1 };
        final EvoAlg[] eas = getEAs(stds, dimMultiplier, orch);
        ...
        final Sprouter[] sprouters = new Sprouter[] {
                new BasicSprouter(4.5, stds[1] / dimMultiplier, state.getPopulationSizes()[1], orch.rand),
                new BasicSprouter(3.5, stds[2] / dimMultiplier, state.getPopulationSizes()[2], orch.rand),
                new NoSprouter()
        };
        final SproutReducer[] sproutReducers = new SproutReducer[] {
                new ChainedSproutReducer(
                        new DemeDistanceSproutReducer(2 * stds[1] / dimMultiplier),
                        new SeedDistanceSproutReducer(2 * stds[1] / dimMultiplier)),
                new ChainedSproutReducer(
                        new DemeDistanceSproutReducer(2 * stds[2] / dimMultiplier),
                        new SeedDistanceSproutReducer(2 * stds[2] / dimMultiplier)),
                new NoSproutReducer()
        };
    ...
    protected abstract EvoAlg[] getEAs(double[] stds, double dimMultiplier, HgsOrchestrator orch);
