 ...
 * </ul>
 * <p>
 * Further modifications by Jakub Sawicki.
 *
 * @version $Id: CMAESOptimizer.java 1573506 2014-03-03 09:58:29Z luc $
 * @since 3.0 of apache math
 */
@Log
public class CMAESOptimizer
    extends MultivariateOptimizer implements EvoAlg {
    ...
    /** Individuals used in the first iteration. */
    private Individual[] inputIndividuals;
...
//    private final int maxIterations;
    @Setter private boolean stopIfSigmaIncrease = false;
     ...
     * @since 3.1
     */
    private CMAESOptimizer(// int maxIterations,
                          double stopFitness,
                          boolean isActiveCMA,
                          int diagonalOnly,
...
//        this.checkFeasableCount = checkFeasableCount;
    ...
    public CMAESOptimizer(double stopFitness, Random random, IndividualEvaluator ie, FitnessExtractor fe) {
        this(stopFitness, true, 0, RandomGeneratorFactory.createRandomGenerator(random), false, null, ie, fe);
    }
    ...
    public static class FirstPopulation implements OptimizationData {
        @Getter
    ...
    public PointValuePair optimize(OptimizationData... optData)
        ...
        throw new NotImplementedException("This method shouldn't be called in this implementation");
    }

    /**
     * Do nothing. The control will come from outside.
     */
    @Override
    ...
    /**
     * This method will initialize the optimizer state, by calling super.optimize,
     * but it won't perform any actual optimization, unlike original suggests
     * {@link MultivariateOptimizer#optimize(OptimizationData...)}.
     *
     ...
     * </ul>
     * @return local variables state
     * @throws TooManyEvaluationsException if the maximal number of
     * evaluations is exceeded.
     * @throws DimensionMismatchException if the initial guess, target, and weight
     * arguments have inconsistent dimensions.
     */
    public void initialize(OptimizationData... optData) {
        if (initialized) {
            throw new IllegalStateException("This instance has already been initialized");
        }
        // Set up base class, but only perform the initialization below
        ...
        if (getStartPoint() != null) {
            ...
            initializeCMA();
            bestValue = ie.evaluate(guess)[0];
            push(fitnessHistory, bestValue);
            optimum = new PointValuePair(getStartPoint(), isMinimize ? bestValue : -bestValue);
        } else {
            final Individual[] inds = inputIndividuals;
            dimension = inputIndividuals[0].getPoint().length;
            bestValue = Double.MAX_VALUE;
            double[] bestPoint = null;
            for (final Individual ind: inds) {
                final double obj = ind.getObjectives()[0];
                if (obj < bestValue) {
                    bestValue = obj;
                    bestPoint = ind.getPoint();
                }
            }
            optimum = new PointValuePair(bestPoint, isMinimize ? bestValue : -bestValue);
            initializeCMA();
            push(fitnessHistory, bestValue);
        }
        initialized = true;
    ...
    public Population runOneStep() {
        if (!initialized) {
            throw new IllegalStateException("To perform a step the instance must be initialized first");
        }
        incrementIterationCount();

        // populate the boolean here to direct the control flow below
        final boolean isInitialRunWithInputPopulation = getStartPoint() == null && iterations.getCount() == 1;

        final double[] fitness;
        final Population population;
        final RealMatrix arz;
        final RealMatrix arx;
        if (!isInitialRunWithInputPopulation) {
            // Generate and evaluate lambda offspring
            arz = randn1(dimension, lambda);
            arx = zeros(dimension, lambda);
            population = new Population(lambda);
            ...
            for (int k = 0; k < lambda; k++) {
                ...
                individuals[k] = new SimpleIndividual(arx.getColumn(k));
            }
            // Compute fitnesses.
            ie.evaluate(population);
        } else {
            arz = randn1(dimension, inputIndividuals.length);
            arx = zeros(dimension, inputIndividuals.length);
            for (int k = 0; k < inputIndividuals.length; k++) {
                final RealMatrix arxk = MatrixUtils.createColumnRealMatrix(inputIndividuals[k].getPoint());
                copyColumn(arxk, 0, arx, k);
            }
            population = new Population(inputIndividuals);
        }
        fitness = fe.extractFitness(population);
...
//            optimum = new PointValuePair(fitfun.repair(bestArx.getColumn(0)),
            ...
            if (getConvergenceChecker() != null && lastResult != null &&
                getConvergenceChecker().converged(iterations.getCount(), optimum, lastResult)) {
                return null;
            }
        }
...
//            final double stopFitness = 1e-3;
        ...
        {
            val indCount = isInitialRunWithInputPopulation ? inputIndividuals.length : lambda;
            if (bestValue == fitness[arindex[(int)(0.1+indCount/4.)]]) {
                if (stopIfSigmaIncrease) {
                    return null;
                } else {
        ...
        if (iterations.getCount() > 2 && FastMath.max(historyWorst, bestFitness) -
            FastMath.min(historyBest, bestFitness) == 0) {
            sigma *= FastMath.exp(0.2 + cs / damps);
        }
        // store best in history
        push(fitnessHistory,bestFitness);
        log.info(String.format("Best fitness: %e", bestFitness));
    ...
    protected void parseOptimizationData(OptimizationData... optData) {
        ...
        for (final OptimizationData data : optData) {
            ...
            if (data instanceof FirstPopulation) {
                inputIndividuals = ((FirstPopulation) data).getIndividuals();
            }
        }
    ...
    private void checkParameters() {
        ...
        if (getStartPoint() != null && inputIndividuals != null) {
            throw new IllegalArgumentException("Only Guess or FirstPopulation may be set at once, not both");
        }
    }
    ...
    private void initializeCMA() {
        ...
        // initialize sigma
        final double[][] sigmaArray = new double[dimension][1];
        for (int i = 0; i < dimension; i++) {
        ...
        double sumwq = 0;
        for (int i = 0; i < mu; i++) {
            final double w = weights.getEntry(i, 0);
            sumw += w;
            sumwq += w * w;
        }
        ...
        damps = (1 + 2 * FastMath.max(0, FastMath.sqrt((mueff - 1) /
                                                       (dimension + 1)) - 1)) *
            /*FastMath.max(0.3,
                         1 - dimension / (1e-6 + maxIterations))*/1. // default to 1., as number of iterations is unknown a'priori in HGS case
                         + cs; // minor increment
        ...
        if (getStartPoint() != null) {
            xmean = MatrixUtils.createColumnRealMatrix(getStartPoint()); // objective variables
        } else {
            xmean = MatrixUtils.createColumnRealMatrix(optimum.getPoint());
        }
    ...
    private boolean updateEvolutionPaths(RealMatrix zmean, RealMatrix xold) {
        ...
        final boolean hsig = normps /
            FastMath.sqrt(1 - FastMath.pow(1 - cs, 2 * iterations.getCount())) /
    ...
    private void updateCovarianceDiagonalOnly(boolean hsig,
        ...
        if (diagonalOnly > 1 &&
            iterations.getCount() > diagonalOnly) {
    ...
    private void updateBD(double negccov) {
        if (ccov1 + ccovmu + negccov > 0 &&
            (iterations.getCount() % 1. / (ccov1 + ccovmu + negccov) / dimension / 10.) < 1) {
            // to achieve O(N^2)
